...
{{-- HERO --}}
{{-- ─────────────────────────────────────────── --}}
<section class="relative flex min-h-screen items-center bg-[#ECF39E] dark:bg-[#132A13] overflow-hidden">
    {{-- Background grid --}}
    ...
    {{-- Glow --}}
    <div class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-128 w-lg rounded-full bg-[#4F772D]/20 dark:bg-[#4F772D]/30 blur-3xl pointer-events-none"></div>
    ...
    <div class="relative w-full"
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-10 lg:gap-16 items-center">
                        ...
                        <div class="space-y-7">
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <span class="inline-flex items-center rounded-full bg-[#31572C] dark:bg-[#1e4a1e] px-3 py-1 text-xs font-semibold text-white dark:text-[#c8de70]">
                                    {{ $slide->classification->name }}
                                </span>
                                @endif
                                @if($slide->category?->name)
                                <span class="inline-flex items-center rounded-full bg-[#4F772D] dark:bg-[#4F772D] px-3 py-1 text-xs font-semibold text-white dark:text-[#ECF39E]">
                                    {{ $slide->category->name }}
                                </span>
                                @endif
                                <span class="inline-flex items-center rounded-full border border-[#90A955] dark:border-[#2a5c2a] bg-[#c8de70] dark:bg-[#2a5c2a] px-3 py-1 text-xs font-semibold text-[#132A13] dark:text-[#c8de70]">
                                    {{ $slide->created_at->format('M d, Y') }}
                                </span>
                            </div>

                            {{-- Title --}}
                            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold tracking-tight text-[#132A13] dark:text-[#ECF39E] leading-tight">
                                {{ $slide->title }}
                            </h1>

                            {{-- Excerpt --}}
                            @if($slide->excerpt)
                            <p class="text-lg leading-relaxed text-[#31572C] dark:text-[#c8de70] max-w-lg">
                            ...
                            <div class="flex flex-wrap items-center gap-4">
                                <a href="{{ route('content.show', $slide->slug) }}"
                                   class="inline-flex items-center gap-2 rounded-xl bg-[#132A13] dark:bg-[#ECF39E] px-6 py-3 text-sm font-semibold text-white dark:text-[#132A13] shadow-lg hover:opacity-90 transition-opacity duration-200">
                                    Read More
                                ...
                                <button @click="preview = true"
                                        class="inline-flex items-center gap-2.5 rounded-xl border-2 border-[#90A955] dark:border-[#4F772D] px-6 py-3 text-sm font-semibold text-[#31572C] dark:text-[#c8de70] hover:bg-[#c8de70] dark:hover:bg-[#2a5c2a] transition-colors duration-200">
                                    <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-[#132A13] dark:bg-[#ECF39E]">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-3 w-3 text-white dark:text-[#132A13] translate-x-0.5">
                        ...
                        <div class="relative order-first lg:order-0">
                            {{-- Blob glow --}}
                            <div class="absolute inset-0 -z-10 scale-110 rounded-3xl bg-[#90A955]/20 dark:bg-[#4F772D]/25 blur-2xl"></div>

                            {{-- Browser card --}}
                            <div class="relative overflow-hidden rounded-2xl border border-[#a0c84a] dark:border-[#2a5c2a] bg-[#f2fad8] dark:bg-[#1e4a1e] shadow-2xl dark:shadow-[0_25px_60px_rgba(0,0,0,0.5)]">
                                {{-- Browser chrome --}}
                                <div class="flex items-center gap-1.5 border-b border-[#a0c84a] dark:border-[#2a5c2a] bg-[#d4eb88] dark:bg-[#1a3a1a] px-4 py-3">
                                    <span class="h-3 w-3 rounded-full bg-red-400"></span>
                                    <span class="h-3 w-3 rounded-full bg-[#90A955]"></span>
                            ...
                            <div class="absolute -bottom-3 -left-3 hidden sm:flex items-center gap-2 rounded-xl bg-[#132A13] dark:bg-[#ECF39E] px-4 py-2 shadow-lg">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-3.5 w-3.5 text-white dark:text-[#132A13] shrink-0">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632Z"/>
                                </svg>
                                <span class="text-xs font-semibold text-white dark:text-[#132A13]">{{ $slide->user?->name ?? $siteSetting->site_title }}</span>
                            </div>

                            {{-- Featured badge (top-right) --}}
                            <div class="absolute -top-3 -right-3 hidden sm:flex items-center gap-1.5 rounded-xl bg-[#4F772D] dark:bg-[#4F772D] px-4 py-2 shadow-lg">
        ...
        <div class="absolute bottom-8 left-1/2 z-10 flex -translate-x-1/2 items-center gap-2">
            ...
            <button @click="go({{ $i }})"
                    :class="current === {{ $i }} ? 'w-6 bg-[#4F772D] dark:bg-[#90A955]' : 'w-2 bg-[#90A955] dark:bg-[#4F772D]'"
                    class="h-2 rounded-full transition-all duration-300"></button>
            @endforeach
        </div>

        {{-- Prev arrow --}}
        <button @click="prev()" class="absolute left-4 top-1/2 z-10 -translate-y-1/2 rounded-full border border-[#a0c84a] dark:border-[#2a5c2a] bg-white/70 dark:bg-[#1e4a1e]/70 p-2.5 text-[#31572C] dark:text-[#c8de70] shadow-md backdrop-blur hover:bg-[#4F772D] hover:text-white dark:hover:bg-[#4F772D] transition-all duration-200">
        ...
        {{-- Next arrow --}}
        <button @click="next()" class="absolute right-4 top-1/2 z-10 -translate-y-1/2 rounded-full border border-[#a0c84a] dark:border-[#2a5c2a] bg-white/70 dark:bg-[#1e4a1e]/70 p-2.5 text-[#31572C] dark:text-[#c8de70] shadow-md backdrop-blur hover:bg-[#4F772D] hover:text-white dark:hover:bg-[#4F772D] transition-all duration-200">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="h-4 w-4">
        ...
        <div x-show="preview"
            ...
            <div x-show="preview"
                 ...
                 class="relative w-full max-w-lg rounded-2xl bg-[#f2fad8] dark:bg-[#1e4a1e] border border-[#a0c84a] dark:border-[#2a5c2a] shadow-2xl p-8">

                {{-- Close --}}
                <button @click="preview = false"
                        class="absolute top-4 right-4 rounded-lg p-1.5 text-[#4F772D] dark:text-[#90A955] hover:bg-[#c8de70] dark:hover:bg-[#2a5c2a] transition-colors">
                ...
                {{-- Category badge --}}
                <div class="mb-4">
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-[#c8de70] dark:bg-[#2a5c2a] px-3 py-1 text-xs font-semibold text-[#31572C] dark:text-[#90A955]">
                        <span class="h-1.5 w-1.5 rounded-full bg-[#4F772D] dark:bg-[#90A955]"></span>
                        <span x-text="slideData[current]?.category ?? 'Featured'"></span>
                    </span>
                </div>

                {{-- Title --}}
                <h2 class="mb-3 text-xl font-bold text-[#132A13] dark:text-[#ECF39E] leading-snug"
                    x-text="slideData[current]?.title"></h2>

                {{-- Excerpt --}}
                <p class="mb-5 text-sm leading-relaxed text-[#31572C] dark:text-[#c8de70]"
                   x-text="slideData[current]?.excerpt || 'No preview available.'"></p>

                {{-- Date --}}
                <p class="mb-6 text-xs text-[#4F772D] dark:text-[#90A955]" x-text="slideData[current]?.date"></p>

                {{-- CTA --}}
                <a :href="slideData[current]?.url"
                   class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#4F772D] dark:bg-[#4F772D] px-5 py-3 text-sm font-semibold text-white dark:text-[#ECF39E] hover:bg-[#31572C] dark:hover:bg-[#6B9A38] transition-colors">
                    Read Full Article
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/>
                    </svg>
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-32 text-center">
        <p class="animate-fade-up mb-4 inline-block rounded-full border border-[#90A955] dark:border-[#4F772D] bg-[#c8de70] dark:bg-[#4F772D]/20 px-4 py-1 text-sm font-medium text-[#31572C] dark:text-[#90A955]">
            Welcome
        </p>
        <h1 class="animate-fade-up-delay-1 text-4xl font-bold tracking-tight text-[#132A13] dark:text-[#ECF39E] sm:text-6xl lg:text-7xl">
            {{ $siteSetting->site_title }}
        </h1>
        <p class="animate-fade-up-delay-2 mt-6 max-w-2xl mx-auto text-lg leading-8 text-[#31572C] dark:text-[#c8de70]">
        ...
        <div class="animate-fade-up-delay-2 mt-10">
            <a href="#content" class="inline-flex items-center gap-2 rounded-xl bg-[#4F772D] dark:bg-[#4F772D] px-7 py-3.5 text-sm font-semibold text-white dark:text-[#ECF39E] shadow-lg hover:bg-[#31572C] dark:hover:bg-[#6B9A38] transition-colors duration-200">
                Browse Content
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4">
...
<section class="relative overflow-hidden bg-[#132A13] dark:bg-[#0a1a0a] py-20 sm:py-24">
    ...
    <div class="absolute -bottom-20 right-1/4 h-72 w-72 rounded-full bg-[#4F772D]/25 blur-3xl pointer-events-none"></div>
    {{-- Subtle grid --}}
    ...
    <div class="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        {{-- Icon --}}
        <div class="mb-6 inline-flex h-16 w-16 items-center justify-center rounded-2xl border border-[#4F772D]/30 bg-[#4F772D]/15">
        ...
        <form method="GET" action="{{ route('search') }}" class="mb-10">
            <div class="flex overflow-hidden rounded-2xl border-2 border-[#4F772D]/30 bg-white/8 backdrop-blur-sm focus-within:border-[#4F772D]/60 transition-all duration-200 shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
                ...
                <button type="submit"
                        class="m-2 rounded-xl bg-[#4F772D] px-6 py-3 text-sm font-semibold text-white shadow-md hover:bg-[#4F772D] transition-colors duration-200 shrink-0">
                    Search
                </button>
            </div>
        </form>
...
<section id="cat-section" class="relative bg-[#d4eb88] dark:bg-[#1a3a1a] py-16 overflow-hidden">
    {{-- Dot grid --}}
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10">
            <div class="flex items-center gap-3 mb-1.5">
                ...
                <h2 class="text-2xl font-bold text-[#132A13] dark:text-[#ECF39E]">Browse by Category</h2>
            </div>
            <p class="ml-9 text-sm text-[#4F772D] dark:text-[#90A955]">Explore our content organised by topic</p>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            <a href="{{ route('category.show', $category->slug) }}"
               class="card-animate group relative overflow-hidden rounded-2xl col-span-2
                      ...
                      dark:bg-[#1e4a1e] border border-transparent dark:border-[#2a5c2a]
                      shadow-sm transition-all duration-300
                      hover:-translate-y-1.5 hover:shadow-[0_8px_30px_rgba(202,138,4,0.25)]
                      dark:hover:shadow-[0_8px_30px_rgba(140,90,60,0.4)]
                      hover:border-[#90A955] dark:hover:border-[#4F772D]"
               :style="darkMode ? {} : { backgroundColor: '{{ $bg }}' }">

                {{-- Left: icon --}}
                <div class="flex items-center justify-center p-6 sm:w-40 shrink-0">
                    <div class="relative">
                        <div class="absolute inset-0 rounded-full bg-[#4F772D]/20 dark:bg-[#4F772D]/15 scale-110 group-hover:scale-125 blur-md transition-transform duration-500"></div>
                ...
                <div class="flex flex-1 flex-col justify-center px-5 pb-5 sm:pl-0 sm:py-5 sm:pr-6">
                    <span class="mb-1.5 text-xs font-bold uppercase tracking-widest text-[#4F772D] dark:text-[#90A955]">Featured</span>
                    <h3 class="text-lg font-bold text-[#132A13] dark:text-[#ECF39E] mb-1.5 leading-tight">{{ $category->name }}</h3>
                    @if($category->description)
                    <p class="text-xs text-[#31572C] dark:text-[#90A955] leading-relaxed line-clamp-2 mb-3">{{ $category->description }}</p>
                    @else
                    <div class="mb-3"></div>
                    @endif
                    <div class="flex items-center gap-2">
                        <span class="inline-flex items-center rounded-full bg-[#4F772D] dark:bg-[#4F772D] px-3 py-1 text-xs font-semibold text-white shadow-sm">
                            {{ $category->contents_count }} {{ $category->contents_count === 1 ? 'article' : 'articles' }}
                        </span>
                        <span class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-white/50 dark:bg-[#2a5c2a]/50
                                     group-hover:bg-[#4F772D] dark:group-hover:bg-[#4F772D] transition-all duration-200">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"
                                 class="h-3 w-3 text-[#31572C] dark:text-[#c8de70] group-hover:text-white group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-200">
            ...
            <a href="{{ route('category.show', $category->slug) }}"
               ...
               :style="darkMode ? {} : { backgroundColor: '{{ $bg }}' }">
                ...
                {{-- Name --}}
                <h3 class="text-sm font-bold text-[#132A13] dark:text-[#ECF39E] leading-tight mb-1">{{ $category->name }}</h3>

                {{-- Description --}}
                @if($category->description)
                <p class="text-xs text-[#31572C] dark:text-[#90A955] leading-relaxed line-clamp-2 mb-2 px-1">{{ $category->description }}</p>
                @else
                <div class="mb-2"></div>
                @endif
                ...
                <span class="inline-flex items-center rounded-full border border-[#4F772D]/30 dark:border-[#4F772D]
                             ...
                             dark:group-hover:bg-[#4F772D] dark:group-hover:text-[#ECF39E]
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-10">
            <div class="flex items-center gap-3 flex-1">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-6 w-6 text-[#4F772D] dark:text-[#4F772D] shrink-0">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/>
                </svg>
                <h2 class="text-2xl font-bold text-[#132A13] dark:text-[#ECF39E]">
            ...
            <form method="GET" action="{{ route('search') }}" class="flex items-center gap-2 w-full sm:w-auto sm:min-w-72">
                <div class="relative flex-1">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                         class="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-[#4F772D] dark:text-[#90A955] pointer-events-none">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"/>
                    </svg>
                    <input type="text" name="q" value="{{ $search }}" maxlength="100" placeholder="Search articles…"
                           class="w-full rounded-xl border border-[#a0c84a] dark:border-[#2a5c2a] bg-[#f2fad8] dark:bg-[#1e4a1e] pl-9 pr-4 py-2.5 text-sm text-[#132A13] dark:text-[#ECF39E] placeholder-[#A87850] dark:placeholder-[#90A955] focus:outline-none focus:border-[#4F772D] dark:focus:border-[#4F772D] transition-colors">
                </div>
                <button type="submit" class="rounded-xl bg-[#4F772D] dark:bg-[#4F772D] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#31572C] dark:hover:bg-[#6B9A38] transition-colors shrink-0">Search</button>
                @if($search)
                <a href="{{ route('search') }}" class="rounded-xl border border-[#a0c84a] dark:border-[#2a5c2a] px-4 py-2.5 text-sm text-[#4F772D] dark:text-[#90A955] hover:bg-[#c8de70] dark:hover:bg-[#2a5c2a] transition-colors shrink-0">Clear</a>
                @endif
            </form>
        </div>

        @if($search)
        <p class="mb-6 text-sm text-[#4F772D] dark:text-[#90A955]">
            {{ $latestContents->total() }} {{ Str::plural('result', $latestContents->total()) }} for
            <span class="font-semibold text-[#132A13] dark:text-[#ECF39E]">"{{ $search }}"</span>
        </p>
        @endif
        ...
        @if($latestContents->isEmpty())
            <div class="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p class="text-[#A87850] dark:text-[#90A955] text-lg">No content published yet.</p>
            </div>
        @else
            <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <article class="group flex flex-col rounded-2xl overflow-hidden bg-[#f2fad8] dark:bg-[#1e4a1e] border border-[#a0c84a] dark:border-[#2a5c2a] shadow-sm hover:shadow-xl dark:hover:shadow-[0_20px_40px_rgba(0,0,0,0.4)] hover:-translate-y-1 transition-all duration-300">
                    <div class="relative aspect-video overflow-hidden bg-[#c8de70] dark:bg-[#2a5c2a]">
                        ...
                        @if($content->category)
                        <span class="absolute top-3 left-3 rounded-full bg-[#4F772D] dark:bg-[#4F772D] px-3 py-1 text-xs font-semibold text-white">
...
<section class="relative py-16 overflow-hidden bg-[#132A13] dark:bg-[#0d2010]">
    {{-- Subtle grid texture --}}
    ...
    {{-- Accent glows --}}
    <div class="absolute -top-20 -right-20 h-72 w-72 rounded-full bg-[#4F772D]/10 blur-3xl pointer-events-none"></div>
    <div class="absolute -bottom-20 -left-20 h-72 w-72 rounded-full bg-[#4F772D]/25 blur-3xl pointer-events-none"></div>
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10">
            <div class="flex items-center gap-3 mb-1.5">
                <div class="flex h-9 w-9 items-center justify-center rounded-xl border border-[#4F772D]/30 bg-[#4F772D]/15">
            ...
            <a href="{{ route('classification.show', $classification->slug) }}"
               class="card-animate group flex items-center gap-3 rounded-2xl p-3
                ...
                <div class="relative shrink-0">
                    <div class="absolute inset-0 rounded-xl bg-[#4F772D]/20 scale-110 group-hover:scale-125 blur-sm transition-transform duration-500 pointer-events-none"></div>
                    <div class="relative h-10 w-10 rounded-xl bg-[#2a5c2a] flex items-center justify-center
                                group-hover:bg-[#4F772D] transition-colors duration-300">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <span class="mt-1 inline-flex items-center rounded-full bg-[#4F772D]/20 border border-[#4F772D]/30 px-2 py-0.5 text-xs font-semibold text-[#90A955]">
                ...
                {{-- Arrow when no image --}}
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                     class="h-4 w-4 shrink-0 text-[#4F772D] group-hover:text-[#90A955] group-hover:translate-x-1 transition-all duration-200">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/>
                </svg>
                @endif
            </a>
...
<section class="relative bg-[#ECF39E] dark:bg-[#132A13] py-16 overflow-hidden">
    {{-- Decorative blobs --}}
    <div class="absolute -top-20 right-0 h-72 w-72 rounded-full bg-[#90A955]/10 dark:bg-[#4F772D]/10 blur-3xl pointer-events-none"></div>
    <div class="absolute bottom-0 left-1/3 h-48 w-48 rounded-full bg-[#FFDAC4]/40 dark:bg-[#1e4a1e]/30 blur-3xl pointer-events-none"></div>
    ...
    <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-1.5">
                ...
                <h2 class="text-2xl font-bold text-[#132A13] dark:text-[#ECF39E]">Most Popular</h2>
            </div>
            <p class="ml-9 text-sm text-[#4F772D] dark:text-[#90A955]">The most read articles right now</p>
        </div>
            ...
            <a href="{{ route('content.show', $top->slug) }}"
               class="card-animate group relative overflow-hidden rounded-2xl min-h-72 lg:min-h-full
                ...
                <div class="absolute top-4 left-4 flex items-center gap-2">
                    <span class="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[#4F772D] text-sm font-black text-white shadow-lg ring-2 ring-white/20">1</span>
                </div>
                ...
                <div class="absolute bottom-0 left-0 right-0 p-6">
                    ...
                    <div class="flex items-center gap-4 text-xs text-white/75">
                        ...
                        <span class="flex items-center gap-1.5">
                            <span class="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#4F772D]/80 text-white text-xs font-bold shrink-0">
            ...
            <div class="flex flex-col gap-3">
                ...
                <a href="{{ route('content.show', $item->slug) }}"
                   class="popular-item group flex items-center gap-4 rounded-2xl p-3.5
                          bg-[#f2fad8] dark:bg-[#1e4a1e]
                          border border-[#a0c84a] dark:border-[#2a5c2a]
                          hover:border-[#90A955] dark:hover:border-[#4F772D]
                   ...
                   style="opacity:0;transform:translateX(20px)">
                    ...
                    <span class="text-3xl font-black leading-none select-none shrink-0 w-8 text-center
                                 ...
                                 group-hover:text-[#90A955] dark:group-hover:text-[#4F772D]
                                 transition-colors duration-300">
                        {{ $rank }}
                    </span>

                    {{-- Thumbnail --}}
                    <div class="relative w-20 h-16 shrink-0 overflow-hidden rounded-xl bg-[#c8de70] dark:bg-[#2a5c2a]">
                        <img src="{{ asset("storage/{$item->header_image}") }}"
                             alt="{{ $item->title }}"
                             loading="lazy"
                             class="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500">
                        {{-- Hover shimmer overlay --}}
                        <div class="absolute inset-0 bg-[#4F772D]/0 group-hover:bg-[#4F772D]/10 transition-colors duration-300 rounded-xl"></div>
                    </div>

                    {{-- Info --}}
                    <div class="flex-1 min-w-0">
                        <h3 class="text-sm font-bold text-[#132A13] dark:text-[#ECF39E] line-clamp-2 leading-snug
                                   group-hover:text-[#31572C] dark:group-hover:text-[#90A955]
                                   transition-colors duration-200 mb-2">
                            {{ $item->title }}
                        </h3>
                        <div class="flex items-center gap-2 text-xs text-[#4F772D] dark:text-[#90A955]">
                            @if($item->user)
                            <span class="inline-flex items-center gap-1 shrink-0">
                                <span class="inline-flex h-4 w-4 items-center justify-center rounded-full bg-[#4F772D]/80 dark:bg-[#4F772D] text-white text-xs font-bold leading-none shrink-0">
                            ...
                            <span class="flex items-center gap-1 shrink-0">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="h-3 w-3 text-[#4F772D] dark:text-[#90A955]">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/>
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                </svg>
                    ...
                    {{-- Arrow --}}
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"
                         class="h-4 w-4 shrink-0 text-[#90A955] dark:text-[#4F772D]
                                group-hover:text-[#4F772D] dark:group-hover:text-[#90A955]
