...
<section class="relative overflow-hidden bg-[#132A13] dark:bg-[#0a1a0a] pt-28 pb-16">
    ...
    <div class="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        {{-- Label + title --}}
        @if($query)
        <p class="mb-2 text-sm font-medium uppercase tracking-widest text-[#4F772D]">Search results for</p>
        ...
        <form method="GET" action="{{ route('search') }}">
            <div class="flex overflow-hidden rounded-2xl border-2 border-[#4F772D]/30 bg-white/8 backdrop-blur-sm focus-within:border-[#4F772D]/60 transition-all duration-200 shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
                ...
                @if($query)
                <a href="{{ route('search') }}"
                   class="self-center mr-2 rounded-lg p-2 text-[#4F772D] hover:text-[#90A955] transition-colors"
                   title="Clear search">
...
<section class="bg-[#ECF39E] dark:bg-[#132A13] py-16">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h2 class="mb-3 text-2xl font-bold text-[#132A13] dark:text-[#ECF39E]">No results found</h2>
            <p class="mb-8 max-w-md text-[#4F772D] dark:text-[#90A955]">
                We couldn't find anything matching <span class="font-semibold text-[#132A13] dark:text-[#ECF39E]">"{{ $query }}"</span>.
            ...
            <div class="mb-8">
                <p class="mb-4 text-sm font-medium text-[#4F772D] dark:text-[#90A955] uppercase tracking-wider">Browse Categories</p>
                <div class="flex flex-wrap justify-center gap-2">
                    @foreach($suggestions as $cat)
                    <a href="{{ route('category.show', $cat->slug) }}"
                       class="rounded-full border border-[#90A955] dark:border-[#4F772D] bg-[#f2fad8] dark:bg-[#1e4a1e] px-4 py-2 text-sm font-medium text-[#31572C] dark:text-[#c8de70] hover:bg-[#4F772D] hover:border-[#4F772D] hover:text-white transition-all duration-200">
                        {{ $cat->name }}
                    </a>
                    @endforeach
                </div>
        ...
        <div class="mb-8 flex items-center justify-between">
            <p class="text-sm text-[#4F772D] dark:text-[#90A955]">
                Showing <span class="font-semibold text-[#132A13] dark:text-[#ECF39E]">{{ $results->firstItem() }}–{{ $results->lastItem() }}</span>
                of <span class="font-semibold text-[#132A13] dark:text-[#ECF39E]">{{ $results->total() }}</span> results
            </p>
            <a href="{{ route('home') }}" class="text-sm text-[#4F772D] dark:text-[#90A955] hover:text-[#31572C] dark:hover:text-[#90A955] transition-colors">
                ← Back to Home
            </a>
        </div>
