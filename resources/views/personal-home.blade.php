...
<meta name="description" content="{{ $siteSetting->site_description ?: $member->position }}">
<meta property="og:title" content="{{ $siteSetting->site_title }}">
<meta property="og:description" content="{{ $siteSetting->site_description ?: $member->position }}">
...
{{-- ── HERO ── --}}
<section id="member-hero" class="relative flex flex-col justify-center bg-(--dark-section) overflow-hidden">
    ...
    <div class="relative mx-auto max-w-7xl w-full px-4 sm:px-6 lg:px-8 pt-52 sm:pt-44 lg:pt-56 pb-28 lg:pb-32">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-[1fr_280px_1fr] gap-8 lg:gap-10 items-center">
            ...
            <div id="member-left" class="flex flex-col gap-4 text-center lg:text-right order-2 lg:order-1">
                ...
                {{-- Eyebrow: show site tagline instead of "Team" --}}
                ...
                <div class="flex justify-center lg:justify-end">
                    <span class="inline-flex items-center gap-2 rounded-full border border-(--accent)/50 bg-(--dark-section)/70 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-(--accent-on-dark) backdrop-blur-sm">
                        <span class="h-1.5 w-1.5 rounded-full bg-(--accent-on-dark) animate-pulse"></span>
            ...
            <div id="member-right" class="flex flex-col gap-4 text-center lg:text-left order-3">
                ...
                {{-- Action buttons: Contact + PDF export --}}
...
{{-- ── CONTACT STRIP ── --}}
@if($siteSetting->contact_email || $siteSetting->contact_address || $siteSetting->contact_phone)
<section class="bg-(--bg-card) border-t border-(--border) py-12">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <h2 class="text-lg font-bold text-(--text-primary) mb-6">{{ __('ui.keep_in_touch') }}</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @if($siteSetting->contact_email)
            <a href="mailto:{{ $siteSetting->contact_email }}"
               class="flex items-center gap-3 rounded-xl border border-(--border) bg-(--bg-primary) px-4 py-3 hover:border-(--accent)/40 hover:-translate-y-0.5 transition-all duration-200">
                ...
                <div class="min-w-0">
                    <p class="text-[10px] text-(--text-muted) uppercase tracking-widest font-bold">Email</p>
                    <p class="text-sm font-medium text-(--text-primary) truncate">{{ $siteSetting->contact_email }}</p>
                </div>
            </a>
            @endif
            ...
            <a href="tel:{{ $siteSetting->contact_phone }}"
               class="flex items-center gap-3 rounded-xl border border-(--border) bg-(--bg-primary) px-4 py-3 hover:border-(--accent)/40 hover:-translate-y-0.5 transition-all duration-200">
                ...
                <div class="min-w-0">
                    <p class="text-[10px] text-(--text-muted) uppercase tracking-widest font-bold">{{ __('ui.contact_phone_label') }}</p>
                    <p class="text-sm font-medium text-(--text-primary) truncate">{{ $siteSetting->contact_phone }}</p>
                </div>
            </a>
            @endif
            ...
            <div class="flex items-center gap-3 rounded-xl border border-(--border) bg-(--bg-primary) px-4 py-3">
                ...
                <div class="min-w-0">
                    <p class="text-[10px] text-(--text-muted) uppercase tracking-widest font-bold">{{ __('ui.contact_info_address') }}</p>
                    <p class="text-sm font-medium text-(--text-primary) line-clamp-2 leading-snug">{{ $siteSetting->contact_address }}</p>
                </div>
            </div>
            @endif
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.addEventListener('scroll', () => {
        ...
        document.getElementById('member-grid').style.transform = `translateY(${offset}px)`;
    }, { passive: true });
